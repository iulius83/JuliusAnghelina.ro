<?php include('layouts/header.php') ?>

<div class="site-wrap">
    <div class="top-text">
        <h2>Architecture: Exhibition Stand Designer</h2>
        <p>As an <strong>exhibition stand designer</strong> I create visually impressive but practical stand design for company displays at international trade fairs. I present my ideas with computer-generated visuals and models, create detailed final renders as well as try as much as possible understand and interpret the correct materials and costs involved in constructing the design. <strong>I also create and integrate the vector and/or raster based artwork included.</strong></p>
        <p>In a little over 1 year I have built more than 20 designs, including a <strong>120sqm-large 1-story-high</strong> proposal for the Bergner Europe stand at <strong>Ambiente Frankfurt 2018.</strong> </p>
        <p>Here are a couple of design examples: </p>
    </div>
    <!-- Slideshow container -->
    <div class="slideshow-container design">

        <!-- Full-width images with number and caption text -->
        <div class="mySlides design">
            <div class="numbertext">1 / 4</div>
            <img src="images/design/exhibition-slide2.jpg" style="width:100%">
            <div class="text">Proposal - Ambiente Frankfurt 2018</div>
        </div>

        <div class="mySlides design">
            <div class="numbertext">2 / 4</div>
            <img src="images/design/exhibition-slide1.jpg" style="width:100%">
            <div class="text">Proposal - Achema Frankfurt 2018</div>
        </div>

        <div class="mySlides design">
            <div class="numbertext">3 / 4</div>
            <img src="images/design/exhibition-slide3.jpg" style="width:100%">
            <div class="text">Proposal - Ambiente Frankfurt 2018</div>
        </div>
        <div class="mySlides design">
            <div class="numbertext">4 / 4</div>
            <img src="images/design/exhibition-slide4.jpg" style="width:100%">
            <div class="text">Proposal - Ambiente Frankfurt 2018</div>
        </div>

        <!-- Next and previous buttons -->
        <a class="prev" onclick="plusSlides(-1)">&#10094;</a>
        <a class="next" onclick="plusSlides(1)">&#10095;</a>
    </div>
    <br>

    <!-- The dots/circles -->
    <div style="text-align:center">
        <span class="dot" onclick="currentSlide(1)"></span>
        <span class="dot" onclick="currentSlide(2)"></span>
        <span class="dot" onclick="currentSlide(3)"></span>
        <span class="dot" onclick="currentSlide(4)"></span>
    </div>

    <div class="top-text">
        <h2>OOH Advertising: Designing Larger-Than-Life Inflatables</h2>
        <p>I started out as a junior graphic designer doing DTP work, creating flyers, banners, billboards, posters - you name it! I loved creating and I wanted more.</p>
        <p>Powerful computers were a luxury back in 2003 when i started out, but long render times didn't set me down. I modeled all the company's existing products in <strong>3D</strong> and created never-before-seen presentations for <strong>Inflatable-Themed Amusement Parks for Children</strong>. This allowed our clients to visualize their possibilites. I remember the low-res renders I did back in 2004-2005 "only" took about 60-80 minutes to complete (only one jpg!).</p>
        <p>It all went downhill from there, I learned more and more and moved up in the company to managing the workflow process, generating brainstorming sessions and exploiting my creativity to the maximum. In the inflatables industry you very rarely to almost never see the same thing again. <strong>Every project was a new challenge.</strong></p>
        <p><strong>The workflow is as follows:</strong></p>
        <div class="inflatables-workflow">
            <img src="images/design/inflatables-workflow.jpg">
        </div>
        <hr>
        <div class="guiness">
            <img src="images/design/sac-ING-23m.jpg" alt="ING Money Sack">
            <p>One notable project was a huge money sack built for a promotion by <strong>ING Romania</strong>. We <em>almost</em> made a <strong>Guiness World Record</strong>-high inflatable: <strong>23m in height! That's almost 6 floors up!</strong> I say <em>almost</em> because we only thought to check the record books <em>after</em> the product was delivered - and the record was 25m! <br> <br><strong>Fun Facts:</strong></p>
            <ul>
                <li>almost <strong>10 km</strong> of raw material used;</li>
                <li>more tham <strong>20 liters</strong> of plotter ink;</li>
                <li>more than <strong>1 tonne</strong> of ballast on the inside;</li>
                <li>mounted on top of a fountain with a <strong>construction crane</strong>;</li>
            </ul> 
        </div>
        <br><br>
        <p>One of the best things in working as a <strong>3D designer and modeler</strong> for <a href="http://www.aztec.ro" style="text-decoration: none; color: black;"><strong>Aztec Inflatables</strong></a> is that I had the opportunity to see my 3D work "come to life" full-size in real life.</p>

        <p>Here are more examples of my work, grab the central slider to see <strong>3D Renders</strong> vs. Real Images:</p>
    </div>
    
    <div class="splitters">
        <div class="image-spliter">
            <div class="mover"></div>
            <img class="img-left" src="images/design/inflatables-pombear-render.jpg">
            <img class="img-right" src="images/design/inflatables-pombear-real.jpg">
        </div>
        <div class="image-spliter">
            <div class="mover"></div>
            <img class="img-left" src="images/design/inflatables-trex-real.jpg">
            <img class="img-right" src="images/design/inflatables-trex-render.jpg">
        </div>
        <div class="image-spliter">
            <div class="mover"></div>
            <img class="img-left" src="images/design/inflatables-cocos-real.jpg">
            <img class="img-right" src="images/design/inflatables-cocos-render.jpg">
        </div>
        <div class="image-spliter">
            <div class="mover"></div>
            <img class="img-left" src="images/design/inflatables-ferrari-real.jpg">
            <img class="img-right" src="images/design/inflatables-ferrari-render.jpg">
        </div>
        <div class="image-spliter">
            <div class="mover"></div>
            <img class="img-left" src="images/design/inflatables-goldenbrau-real.jpg">
            <img class="img-right" src="images/design/inflatables-goldenbrau-render.jpg">
        </div>
        <div class="image-spliter">
            <div class="mover"></div>
            <img class="img-left" src="images/design/inflatables-kinder-real.jpg">
            <img class="img-right" src="images/design/inflatables-kinder-render.jpg">
        </div>
    </div>
</div>

<?php include('layouts/footer.php') ?>